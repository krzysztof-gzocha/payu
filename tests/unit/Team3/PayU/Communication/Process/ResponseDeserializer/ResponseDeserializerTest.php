...
use Team3\PayU\Communication\Response\EmptyResponse;
...
class ResponseDeserializerTest extends \Codeception\TestCase\Test
{
    ...
    {
        $serializer = $this->getMock('Team3\PayU\Serializer\SerializerInterface');
        $serializer
            ->expects($this->any())
            ->method('fromJson')
            ->withAnyParameters()
    ...
    {
        $response = $this
            ->getMock('Team3\PayU\Communication\Response\ResponseInterface');
        $response
            ->expects($this->any())
            ->method('supports')
            ->withAnyParameters()
